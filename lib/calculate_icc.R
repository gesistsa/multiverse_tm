...
tmmv.calculate_icc <- function(x) {
    ## please note that x can't be named list
    ...
    x <- purrr::map(x, as.data.frame) |>
        ...
        purrr::chuck("result")
    ## most of the code below was copied and modified from psych::ICC version 2.5.6
    n.obs <- nrow(x)
    nj <- ncol(x)
    x.s <- stack(x)
    x.df <- data.frame(
        x.s,
        subs = rep(paste("S", seq_len(n.obs), sep = ""), nj)
    )
