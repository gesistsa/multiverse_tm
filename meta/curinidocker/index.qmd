...
In this investigation, we construct a Docker image using the following Dockerfile that should represent the similar, if not the same, the computational environment as what @curini:2021:CMU used for doing their analysis. 

```Dockerfile
{{< include Dockerfile >}}
```

With this Docker image, we launch a Docker container (Except Step 5) to run the original code and data shared by @curini:2021:CMU on Dataverse. We modify one thing at a time, until we find out the discrepancy in the estimate. We can then establish causal link between what has been changed and the irreproducibility of the originally reported estimate by @curini:2021:CMU, i.e. a positive estimate of **0.823**.
