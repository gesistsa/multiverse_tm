...
In Curini & Vignoli, the following regression table was reported (@fig-regression).

![Original regression table in Curini & Vignoli (fair use)](regression.png){#fig-regression}
...
Please note also that the work space dump was also shared:  [https://dataverse.harvard.edu/file.xhtml?fileId=4291438&version=1.0](https://dataverse.harvard.edu/file.xhtml?fileId=4291438&version=1.0). In the dump, among other things, `corpus` and `myDfm` are included. In addition, the generated file `scores_slda.csv` is also shared.

We were asked to investigate what causes the discrepancy (the reported positive value in Curini & Vignoli and the negative values in our multiverse analysis). Indeed, we adopted the code but also introduced several changes.
...
# Step 1: Testing our translated R code
...
As the original workspace dump was provided, we rerun here the analysis from the provided document-term matrix.
...
1. The only line of code added compared to `part2ita.R`. It could be in the initialization file (e.g., `.Rprofile`).
...
In fact, we observed more missing terms. Based on the Italian stemming algorithm, many Italian terms in the original document-term matrix were not properly tokenized. For instance, "penso" (English: I think) should be tokenized to "pens". But it was tokenized into "penso" in the original document-term matrix. It is almost certain that the original authors would have used the default English stemmer to stem Italian texts. It can also be reflected in the terms in the dictionary for seeded LDA: Terms in the dictionary were assumed to be stemmed using the English stemmer.

In addition, we also observed many terms in the original document-term matrix with incorrect encoding (e.g., "grazie<U+00E8>"). It *could* be due to the fact that the original authors did the tokenization on a machine that was not configured properly to handle Unicode (UTF-8).
...
The original code, even without introducing anything new, cannot reproducible the original finding, not least because of tokenization. We cannot (fully) explain the exact reason for irreproducibility of the original code. [^1] But one thing is clear: The regression coefficient changes drastically just by tweaking how one tokenizes or normalizes the texts. This part matches the findings of the multiverse analysis. Also, the original finding of a positive regression coefficient might well be an artefact of machine-specific idiosyncrasies, including how a (misconfigurated) computational environment handles Unicode. 
...
# Testing modified dictionary

The original dictionary has some interesting choices. We have to clean that up so that it can used in other token normalization situations.

The original dictionary does use the wildcard character in some cases ("solidal*", therefore it will match both adjective *solidale*, *solidali*, *solidalmente*), but not in other ("comunit"). We have normalized all of these cases to using the wildcard character.

```{bash}
#docker compose run --remove-orphans --rm r36 Rscript --vanilla "meta/curinidocker/part3.R"
```

This choice can affect the effect estimate. But the direction is still positive.
