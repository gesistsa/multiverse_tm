...
Then, the topic modeling for all `settings` is executed.
...
The idea of anchoring is to find out the column vector in $\theta$ that has the maximum correlation with `anchor_theta` (See equation 2 in the article). So that one can use that column vector to represent the setting used for training this model.
