...
Please note that some code (e.g., the so-called "pretokenization") was from the original code by @tvinnereim:2015:E.
...
We build the function `train_model` that accepts one `setting` and `dfm_list` and do the topic modeling for each `setting` in `settings`.
...
Actually, we do the topic modeling for all `settings`.
...
This `setting` represents the original setting used in @tvinnereim:2015:E. We call it `anchor_setting`.
...
We will then use this `anchor setting` to extract the model trained with this setting.

```{r}
anchor_mod <- models[[rlang::hash(anchor_setting)]]
```

And then we will use the anchor model to perform exactly the same analysis that @tvinnereim:2015:E did, namely, to find out the most distinguish topic cluster that is associated with age. And then extract the vector of topic proportion as `anchor_theta` ($G_{:,A}$ in the article). 
...
The idea of anchoring is to find out the column vector in $\theta$ that has the maximum correlation with `anchor_theta` (See equation 2 in the article).
