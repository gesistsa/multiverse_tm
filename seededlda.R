...
saveRDS(condits, "data/seededlda_condits.RDS")
...
for (i in seq_len(nrow(condits))) {
    ...
    mod <- x %>% textmodel_seededlda(dictionary = un_dict, valuetype = "fixed", residual = condits$extra_ntopics[i], max_iter = condits$niters[i])
    saveRDS(mod, paste0("res/seededlda/", i, ".RDS"))
}
