...
est <- stm::estimateEffect(~ time + selfLoss + connectedness + vastness + physiological + accommodation,
                           stmobj = anchor_mod$mod, metadata = anchor_mod$docvars)
...
max_topic_index <- which.max(
    purrr::map_dbl(est$parameters,
                   \(x) mean(purrr::map_dbl(x, \(y) y$est["time"]))))
...
get_effect_size_mod <- function(setting, anchor_theta) {
    ...
    if (setting$alternative_model) {
        strata_topic <- keyATM::by_strata_DocTopic(current_mod$mod, by_var = "time", by_values = c(4, 5), labels = c(4,5))
        ...
        est <- stm::estimateEffect(~time + selfLoss + connectedness + vastness + physiological + accommodation, stmobj = current_mod$mod, metadata = current_mod$docvars)

        res <- plot(est, covariate = "time",
                    model = current_mod$mod, method = "difference",
                    cov.value1 = 5, cov.value2 = 4,
                    omit.plot = TRUE)
