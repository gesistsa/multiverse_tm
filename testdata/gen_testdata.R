...
dir.create(here::here("testdata/TXT"), recursive = TRUE)

for (i in seq_along(txt_shuffled)) {
    dir.create(here::here("testdata/TXT", dirname(txt_files[i])), showWarnings = FALSE)
    writeLines(txt_shuffled[[i]], here::here("testdata/TXT", txt_files[i]))
}
