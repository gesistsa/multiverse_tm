...
dir.create(here::here("tests/TXT"), recursive = TRUE)

for (i in seq_along(txt_shuffled)) {
    dir.create(here::here("tests/TXT", dirname(txt_files[i])), showWarnings = FALSE)
    writeLines(txt_shuffled[[i]], here::here("tests/TXT", txt_files[i]))
}
