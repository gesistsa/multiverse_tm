...
for (i in seq_len(nrow(combis))) {
    print(i)
    if (combis[i, 2] == 1) {
        temp_tokens <- tokens %>% tokens_select(stopwords("english"), selection = "remove", padding = FALSE, verbose = TRUE)
    } else {
        temp_tokens <- tokens
    }
    ## This tokens_select is kind of unreasonable, but let's keep it
    temp_tokens <- temp_tokens %>% tokens_select(c("[\\d-]", "[[:punct:]]", "^.{1}$", "us", "united_nations", "united", "nations"),
                                                 selection = "remove", 
                                                 valuetype="regex", 
                                                 min_nchar = 2L,
                                                 verbose = TRUE)
    if (combis[i,1] == 2) {
        ...
        temp_tokens <- tokens_replace(temp_tokens, ori_types, lemma_types, valuetype = "fixed")        
    }
    if (combis[i,1] == 3) {
    ...
    if (combis[i,3] == 1) {
        temp_dfm <- temp_dfm %>% dfm_trim(min_docfreq = 0.005, max_docfreq = 0.5, docfreq_type = "prop", verbose = TRUE)
    }
